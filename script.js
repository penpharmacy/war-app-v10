...
function distributeDose(total, option, skipDays) {
  ...
  function generatePlans(index, currentPlan, currentTotal) {
    if (index === availableDays.length) {
    ...
    for (let a of pills) {
      currentPlan[index] = a;
      generatePlans(index + 1, currentPlan, currentTotal + a);
    }
  }

  for (let length = availableDays.length; length >= 1; length--) {
    generatePlans(0, Array(length).fill(0), 0);
    if (plans.length > 0) break; // หยุดที่แผนที่ใช้วันเยอะที่สุดเท่าที่สามารถทำได้
    availableDays.pop(); // ถ้าไม่ได้ผล ลองตัดวันลงทีละวัน
  }

  if (plans.length === 0) return daily;

  const best = plans[0]; // เอาแผนแรกสุดที่ได้
  for (let i = 0; i < best.length; i++) {
    daily[availableDays[i]] = best[i];
  }
  return daily;
}
